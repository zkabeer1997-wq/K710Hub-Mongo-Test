    ...
    <div className={styles.workspace}>
      ...
      <aside className={styles.results}>
        ...
        <section className={styles.next}>
          ...
          <p>{plan.steps[0] ? `Reserve ${fmt(plan.totals.trueGold)} TG for construction. ${firstDay ? `Complete ${firstDay.runs} refinement${firstDay.runs === 1 ? "" : "s"} on schedule day ${firstDay.day}.` : "No affordable refinement is currently scheduled."}` : "Targets update the full cost and refining schedule immediately."}</p>
        </section>
        ...
        <p className={styles.scheduleStatus}>{plan.refining.status === "achievable" ? `Target reached ${plan.refining.earliestDate || `on day ${plan.refining.earliestDay}`}.` : `Target remains short by ${fmt(Math.max(0, plan.totals.temperedTrueGold - plan.refining.finalTempered), 1)} TTG inside this horizon.`}</p>
