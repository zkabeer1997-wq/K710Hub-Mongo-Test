 ...
 * 2. A member session cookie with role admin / superadmin (edge-safe),
 * 3. A full Kingshot Mongo session with elevated role (Node API routes).
 */
export async function isAdminRequest(request) {
  ...
  try {
    const { readMemberSession } = await import('./memberAuth.js');
    const member = await readMemberSession(request);
    if (isElevatedRole(member?.role)) return true;
  } catch {
    /* continue */
  }

  try {
    const { readKingshotSession } = await import('./memberAuthKingshot.js');
    const session = await readKingshotSession(request);
    return isElevatedRole(session?.role);
  } catch {
    return false;
  }
}
