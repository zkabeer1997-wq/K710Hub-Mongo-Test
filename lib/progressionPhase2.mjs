...
export function planTtgProduction(input, recipes = TTG_TIERS) {
  ...
  const schedule = [];
  const initiallyCovered = ttg >= target;
  ...
  for (let day = 1; day <= days; day += 1) {
    if (input.stopAtTarget && ttg >= target) break;
    tg += income;
  ...
  const startDate = input.startDate
    ...
    : new Date();
  const earliestDate = initiallyCovered
    ...
    : null;
  return {
    status: initiallyCovered || reached ? "achievable" : "shortfall",
    earliestDay: initiallyCovered ? 0 : reached?.day || null,
    earliestDate,
  ...
  while (true) {
    const candidates = rows
      ...
      .map((row) => {
        ...
        return {
          ...row,
          actionType: milestone ? 'imbuement' : ascensionMythic > 0 ? 'ascension' : 'enhancement',
          id,
          stat,
          troop,
          level,
