...
export function buildPetRows(petName) {
  ...
  for (let level = 1; level < pet.maxLevel; level += 1) {
    ...
    // Gray Wolf uses its own published food curve. It is not a fixed fraction
    // of the Legendary curve used by the other rarities.
    if (pet.maxLevel === 50) {
      if (next <= 9) base = 150 + (next - 2) * 10;
      else if (next === 10) base = 235;
      else if (next <= 19) base = 250 + (next - 11) * 15;
      else if (next === 20) base = 390;
      else if (next <= 29) base = 410 + (next - 21) * 20;
      else if (next === 30) base = 600;
      else if (next <= 39) base = 630 + (next - 31) * 30;
      else if (next === 40) base = 910;
      else if (next <= 49) base = 950 + (next - 41) * 40;
      else base = 1320;
    }
    ...
    rows.push({
      ...
      fromLevel: level,
      toLevel: next,
      food: pet.maxLevel === 50 ? base : Math.round(base * pet.costScale),
...
function calculateHeroGearPlanCore(rows, inventory) {
  ...
  const packCandidates = buildHeroPackCandidates(rows, levels, masteries, remaining);
  return {
    ...
    .flatMap((row) => {
      ...
      if (level >= 20 && mastery < 20) {
        ...
        candidates.push({
          ...
          costs: {
            xp: 0,
            forgehammers: nextMastery * 10,
            mythicPieces: Math.max(0, nextMastery - 10),
            mithril: 0,
          },
        });
      }
