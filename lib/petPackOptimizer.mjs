...
function resourcePlanForChests(shortfall, ownedChests, boughtChests, weeks, deadline) {
  ...
  let best = null;
  let opCount = 0;
  const useful = Object.fromEntries(['manual', 'potion', 'medal'].map(key => [
    key,
    Math.ceil(shortfall[key] / PET_RESOURCES[key].chestYield),
  ]));
  for (let manualChests = 0; manualChests <= Math.min(available, useful.manual); manualChests++) {
    const afterManual = available - manualChests;
    for (let potionChests = 0; potionChests <= Math.min(afterManual, useful.potion); potionChests++) {
      const afterPotion = afterManual - potionChests;
      for (let medalChests = 0; medalChests <= Math.min(afterPotion, useful.medal); medalChests++) {
        checkDeadline(deadline, ++opCount);
        ...
        const redeemed = manualChests + potionChests + medalChests;
        if (!best || cost < best.cost - 1e-7 || (Math.abs(cost - best.cost) < 1e-7 && redeemed < best.redeemed)) {
          best = {
            cost,
            redeemed,
            unusedChests: available - redeemed,
            allocations: { manual: manualChests, potion: potionChests, medal: medalChests },
            plans,
          };
        }
      }
...
function optimizePetPacks({ need, have, ownedChests = 0, maxWeeks = 12, timeBudgetMs = DEFAULT_TIME_BUDGET_MS }) {
  ...
  if (!best) return timedOut ? { timedOut: true, shortfall } : { infeasible: true, shortfall };
