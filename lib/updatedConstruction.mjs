...
export function calculateUpdatedConstruction(selections = [], inventory = {}, refinement = {}, options = {}) {
  ...
  if (options.includePrerequisites !== false) {
    ...
    for (let index = current + 1; index <= target; index += 1) {
      ...
      if (!prerequisites && index >= 8) {
        warnings.push(`${targetTier} Town Center prerequisites are not included because the supplied workbook does not identify the exact rotating troop building.`);
        continue;
      }
