...
const gearCosts = () => GOVERNOR_GEAR_TIER_COSTS.flatMap((cost,tier)=>cost ? [field(`cost.${tier}.g`,'Satin',cost[0],`Tier ${tier} — ${GOVERNOR_GEAR_OPTIONS[tier]}`,0),field(`cost.${tier}.d`,'Gilded Threads',cost[1],`Tier ${tier} — ${GOVERNOR_GEAR_OPTIONS[tier]}`,0)] : []);
...
export function toolConfiguration(tool, values={}) {
 ...
 if(tool==='governor-gear-sailing-tool')return {costs:GOVERNOR_GEAR_TIER_COSTS.map((c,i)=>c?[q[`cost.${i}.g`],q[`cost.${i}.d`]]:null),rewards:q};
