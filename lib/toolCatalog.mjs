...
const gearCosts = () => GOVERNOR_GEAR_TIER_COSTS.flatMap((cost,tier)=>{
 ...
 return [field(`cost.${tier}.g`,'Satin',cost[0],group,0),field(`cost.${tier}.d`,'Gilded Threads',cost[1],group,0),field(`cost.${tier}.shards`,"Artisan's Visions",cost[2] || 0,group,0)];
});
...
export function toolConfiguration(tool, values={}) {
 ...
 if(tool==='governor-gear-sailing-tool')return {costs:GOVERNOR_GEAR_TIER_COSTS.map((c,i)=>c?[q[`cost.${i}.g`],q[`cost.${i}.d`],q[`cost.${i}.shards`]]:null),rewards:q};
