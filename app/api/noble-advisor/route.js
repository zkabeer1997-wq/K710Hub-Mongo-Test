...
export async function GET(request) {
  ...
  const [{ data: record, error }, { data: profile }] = await Promise.all([
    db.from('noble_advisor_submissions').select('*').eq('member_id', session.memberId).maybeSingle(),
    db.from('power_profiles').select('name').eq('member_id', session.memberId).maybeSingle(),
  ]);
  if (error) return NextResponse.json({ error: 'Unable to load your booking.' }, { status: 500, headers });
  return NextResponse.json(
    { record: record || { in_game_name: profile?.name || '', member_id: session.memberId } },
    { headers }
  );
}
...
export async function POST(request) {
  ...
  const { error: saveError } = await createAdminSupabaseClient()
    .from('noble_advisor_submissions')
    .upsert(
      { ...record, member_id: session.memberId, updated_at: new Date().toISOString() },
