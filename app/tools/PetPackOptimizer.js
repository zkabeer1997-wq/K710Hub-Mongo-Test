...
export default function PetPackOptimizer({configuration}) {
  ...
  useEffect(() => { setNeed(calculatedNeed); setResult(null); }, [calculatedNeed]);
  const persistedInputs = useMemo(() => ({ need, have, ownedChests, maxWeeks, pets }), [need, have, ownedChests, maxWeeks, pets]);
  const restore = useCallback(saved => {
    if (saved?.need) setNeed(current => ({ ...current, ...saved.need }));
  ...
  const persistence = useToolPersistence({ toolKey: 'pet-pack-optimizer', schemaVersion: 2, inputs: persistedInputs, restore, autoDetect: true });
  ...
  return <section className="ppo-shell">
    <div className="ppo-inputs">
      ...
      <div className="ppo-columns">
        <div><h3>Materials required</h3>{Object.keys(resourceNames).map((key, i) => <NumberField key={key} label={resourceNames[key]} value={need[key]} accent={['#86a873','#ef8348','#65a9d8','#d9a94e'][i]} onChange={value => update(setNeed, key, value)} />)}</div>
    ...
    <style>{`
      ...
      @media(max-width:580px){.ppo-inputs{padding:16px}.ppo-panel-head{flex-direction:column}.ppo-columns,.ppo-settings,.ppo-pet-grid{grid-template-columns:1fr}.ppo-result-head dl{grid-template-columns:1fr}.ppo-result-head dl div{border-left:0;border-top:1px solid var(--edge)}.ppo-pack{grid-template-columns:74px minmax(0,1fr)}.ppo-pack em{grid-column:2}.ppo-week-list{padding:12px}}
